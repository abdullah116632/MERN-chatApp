...
const Message = ({message, nextMessage}) => {
  ...
  const formatedTime = shouldShowTimestamp(message, nextMessage) ? extractTime(message.createdAt) : ""
  ...
  return (
    <div className={`chat ${chatClassName}`}>
      ...
      <div className={`chat-bubble text-white ${marginWhenNoProfileForRecever} ${bubbleBgColor}`}>{message.message}</div>
      <div className='chat-footer opacity-50 text-md font-medium flex gap-1 items-center text-gray-900'>{formatedTime}</div>
    </div>
  );
}
